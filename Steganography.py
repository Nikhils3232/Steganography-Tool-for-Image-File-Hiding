...
class SteganographyApp:
    ...
    def setup_embed_tab(self):
        ...
        # Image preview with scrollbars
        self.embed_canvas = tk.Canvas(self.embed_tab, bg="#eee")
        self.embed_canvas.grid(row=1, column=0, columnspan=3, sticky="nsew", padx=5, pady=5)
        self.embed_scrollbar_y = ttk.Scrollbar(self.embed_tab, orient=tk.VERTICAL, command=self.embed_canvas.yview)
        self.embed_scrollbar_y.grid(row=1, column=3, sticky="ns")
        self.embed_scrollbar_x = ttk.Scrollbar(self.embed_tab, orient=tk.HORIZONTAL, command=self.embed_canvas.xview)
        self.embed_scrollbar_x.grid(row=2, column=0, columnspan=3, sticky="ew")
        self.embed_canvas.configure(yscrollcommand=self.embed_scrollbar_y.set, xscrollcommand=self.embed_scrollbar_x.set)
        ...
        # Secret message
        ttk.Label(self.embed_tab, text="Secret Message:").grid(row=3, column=0, sticky="nw", padx=5, pady=5)
        ...
        ttk.Checkbutton(self.embed_tab, text="Encrypt Message", variable=self.encrypt_var,
                        command=self.toggle_encrypt_fields).grid(row=4, column=0, sticky="w", padx=5, pady=5)
        self.password_frame = ttk.Frame(self.embed_tab)
        self.password_frame.grid(row=5, column=0, columnspan=3, sticky="w", padx=5, pady=5)
        ...
        ttk.Button(self.embed_tab, text="Embed Message", command=self.embed_message).grid(row=6, column=0, columnspan=3, pady=10)
    ...
    def setup_extract_tab(self):
        ...
        # Image preview with scrollbars
        self.extract_canvas = tk.Canvas(self.extract_tab, bg="#eee")
        self.extract_canvas.grid(row=1, column=0, columnspan=3, sticky="nsew", padx=5, pady=5)
        self.extract_scrollbar_y = ttk.Scrollbar(self.extract_tab, orient=tk.VERTICAL, command=self.extract_canvas.yview)
        self.extract_scrollbar_y.grid(row=1, column=3, sticky="ns")
        self.extract_scrollbar_x = ttk.Scrollbar(self.extract_tab, orient=tk.HORIZONTAL, command=self.extract_canvas.xview)
        self.extract_scrollbar_x.grid(row=2, column=0, columnspan=3, sticky="ew")
        self.extract_canvas.configure(yscrollcommand=self.extract_scrollbar_y.set, xscrollcommand=self.extract_scrollbar_x.set)
        ...
        ttk.Checkbutton(self.extract_tab, text="Decrypt Message", variable=self.decrypt_var,
                        command=self.toggle_decrypt_fields).grid(row=3, column=0, sticky="w", padx=5, pady=5)
        self.extract_password_frame = ttk.Frame(self.extract_tab)
        self.extract_password_frame.grid(row=4, column=0, columnspan=3, sticky="w", padx=5, pady=5)
        ...
        ttk.Button(self.extract_tab, text="Extract Message", command=self.extract_message).grid(row=5, column=0, columnspan=3, pady=10)

        ttk.Label(self.extract_tab, text="Extracted Message:").grid(row=6, column=0, sticky="nw", padx=5, pady=5)
        self.extracted_message_text = tk.Text(self.extract_tab, height=10, state=tk.DISABLED)
        self.extracted_message_text.grid(row=6, column=1, columnspan=2, sticky="nsew", padx=5, pady=5)
    ...
    def embed_message(self):
        ...
        if self.encrypt_var.get():
            ...
            if not password:
                ...
                return
            message = f"ENCRYPTED:{self.encrypt_message(message, password)}"
        ...
        try:
            ...
            if extracted_data.startswith("ENCRYPTED:"):
                if not self.decrypt_var.get():
                    messagebox.showwarning("Warning", "Encrypted message detected!")
                ...
                if not password:
                    ...
                    return
                decrypted = self.decrypt_message(extracted_data.split("ENCRYPTED:")[1], password)
                if decrypted: self.show_extracted_message(decrypted)
            else:
                self.show_extracted_message(extracted_data)
        except Exception as e:
            messagebox.showerror("Error", str(e))
